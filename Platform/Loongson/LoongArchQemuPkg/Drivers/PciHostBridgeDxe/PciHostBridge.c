  ...
  )
{
  ...
  //DEBUG ((EFI_D_INFO, "Start bus Enumeration in loongarch64!\n"));
  ...
  for (Link = GetFirstNode (&HostBridge->RootBridges)
    ...
    RootBridge = ROOT_BRIDGE_FROM_LINK (Link);
    //DEBUG ((EFI_D_INFO, "start bus enumeration in loongarch64, addrRangeMin: %ld\n", RootBridge->Bus.Base));
    if (RootBridgeHandle == RootBridge->Handle) {
  ...
  for (Link = GetFirstNode (&HostBridge->RootBridges)
    ...
    if (RootBridgeHandle == RootBridge->Handle) {
      ...
      for (Descriptor = (EFI_ACPI_ADDRESS_SPACE_DESCRIPTOR *) Configuration; Descriptor->Desc == ACPI_ADDRESS_SPACE_DESCRIPTOR; Descriptor++) {
        DEBUG ((EFI_D_INFO, "func:%a- line:%d, resource type:%d\n", __func__, __LINE__, Descriptor->ResType));
        if (Descriptor->ResType > ACPI_ADDRESS_SPACE_TYPE_BUS) {
          return EFI_INVALID_PARAMETER;
        }

        DEBUG ((EFI_D_INFO, "func:%a- line:%d\n", __func__, __LINE__));
